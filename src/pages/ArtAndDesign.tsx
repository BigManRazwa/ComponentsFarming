  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <div className="relative">
          <select
            ...
            className="px-4 pr-10 py-2.5 rounded-xl border border-zinc-800/60 bg-zinc-900/30 text-sm text-zinc-400 focus:outline-none focus:border-blue-500/40 cursor-pointer appearance-none min-w-[140px]"
          >
            {sortOptions.map((opt) => (
              <option key={opt} value={opt}>{opt}</option>
            ))}
          </select>
          <svg className="absolute right-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={onGoUpload}
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-blue-500 to-cyan-500 text-white text-sm font-medium hover:from-blue-400 hover:to-cyan-400 active:scale-[0.97] transition-all cursor-pointer"
        >
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          {themes.length === 0 ? (
            <>
              ...
              <p className="text-zinc-500 text-xs mb-4">Upload your first theme to get started.</p>
              <button onClick={onGoUpload} className="text-sm text-blue-400 hover:text-blue-300 transition-colors font-medium cursor-pointer">
                + Upload Design
              </button>
            </>
          ) : (
